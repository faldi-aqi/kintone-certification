...
const functions = ((
  ...
  return {
    ...
    findAssignee: (arr) => {
      ...
      return {assignee: arr[0].Login_Name.value, id: arr[0].$id.value, workload: arr[0].Workload.value};
    },
    ...
    calculateDeadlines: (todayDate) => {
      ...
      const skipWeekends = (date) => {
        while (date.weekday === 6 || date.weekday === 7) { // 6 = Saturday, 7 = Sunday
    ...
    generateBodyRecordsPo: async (e) => {
      ...
      for (const attachment of attachments) {
        ...
        // Example usage:
        const todayDate = lib.dt.utc().toISODate(); // Get today's date in yyyy-mm-dd format
        ...
        const newRecord = {
          ...
          Date: {value: formatDateFromExcel(excelData[3][excelData[3].length - 1])},
        ...
        try {
          ...
          console.log({assignee}, {id}, {workload});
      ...
      async function downloadFileContent(sigFileKey) {
        ...
        const downloadedData = await client.file.downloadFile({fileKey: sigFileKey});
